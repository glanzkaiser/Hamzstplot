...
int main() {
	...
	double m = 3;
	double k= 100; 
	double gamma = sqrt(Gamma*m*k);
	return asin(gamma*x0/sqrt(m*m*pow(1-pow(x0,2),2) +gamma*gamma*pow(x0,2))); }); 
	auto y1 = transform(x, [](double x) { 
	double Gamma = 0.015625;
	double m = 3;
	double k= 100; 
	double gamma = sqrt(Gamma*m*k);
	return pi - asin(gamma*x/sqrt(m*m*pow(1-pow(x,2),2) +gamma*gamma*pow(x,2))) ; }); 
	...
	title("Forced Vibration with Damping, m = 3, k = 100");
	xlabel("ω/ω_{0}");
	ylabel("𝛿");
	xrange({0, 5});
	yrange({0, 5});
